(function () {
  ...
  const styleText = `
    ...
    #glm-coding-guard .guard-title { font-weight: 700; letter-spacing: 0.02em; }
    ...
    #glm-coding-guard input[type="text"] { width: 170px; }
    ...
    #glm-coding-guard .btn-primary { background: #2563eb; color: white; }
    #glm-coding-guard .btn-secondary { background: rgba(148, 163, 184, 0.16); color: #e2e8f0; }
    ...
    #glm-coding-guard .guard-status strong { color: #fef3c7; }
    #glm-coding-guard .guard-switches { display: grid; gap: 8px; }
    ...
    #glm-coding-guard.minimized .guard-body { display: none; }
  `;
  ...
  const config = loadJson(STORAGE_KEY, DEFAULT_CONFIG);
  const persistedState = loadJson(STATE_KEY, { logs: [] });

  function log(message) {
    const timestamp = new Date().toLocaleTimeString('zh-CN', { hour12: false });
    persistedState.logs = [`[${timestamp}] ${message}`, ...(persistedState.logs || [])].slice(0, 12);
    saveJson(STATE_KEY, persistedState);
    const logNode = document.querySelector('#glm-coding-guard .guard-log');
    if (logNode) logNode.textContent = persistedState.logs.join('\n');
  }

  function notify(title, message) {
    const now = Date.now();
    if (now - runtime.notifyAt < config.notificationCooldownMs) return;
    runtime.notifyAt = now;
    if (typeof chrome !== 'undefined' && chrome.runtime?.id && chrome.notifications) {
      chrome.notifications.create({
        type: 'basic',
        iconUrl: 'data:image/png;base64,iVBORw0KGgoAAAANSUhEUgAAAAEAAAABCAQAAAC1HAwCAAAAC0lEQVR42mP8/x8AAusB9s2l9o8AAAAASUVORK5CYII=',
        title,
        message
      });
  ...
  function matchTargets() {
    ...
    for (const node of nodes) {
      ...
      if (!text) continue;
      if (textIncludesAny(text, config.soldOutKeywords)) soldOutSeen = true;
      if (textIncludesAny(text, config.stopKeywords)) stopSeen = true;
    ...
    if (config.observeTexts) {
      ...
      if (textIncludesAny(bodyText, config.stopKeywords)) stopSeen = true;
      if (textIncludesAny(bodyText, config.soldOutKeywords)) soldOutSeen = true;
    }
  ...
  function highlight(node) {
    ...
    setTimeout(() => { node.style.outline = previousOutline; }, 2500);
  }
  ...
  function renderStatus(text) {
    ...
    statusNode.innerHTML =
      ...
      `<div>${text}</div>` +
      `<div>距离下次 10:00 开售：${formatCountdown(msUntilRelease())}</div>`;
  }
  ...
  function tryAction() {
    ...
    if (status.stopSeen) {
      renderStatus('已进入确认/支付阶段，等待你手动确认。');
      stopLoop();
    ...
    renderStatus(status.soldOutSeen ? '当前仍像是未开售/已售罄状态。' : '暂未识别到目标按钮，继续轮询。');
  ...
  function startLoop() {
    ...
    runtime.reloadTimer = setInterval(() => {
      if (config.autoReloadInWindow && withinPurchaseWindow() && !document.hidden) tryAction();
  ...
  function bindConfig(root) {
    root.addEventListener('change', (event) => {
      ...
      if (!key) return;
      if (target.type === 'checkbox') config[key] = target.checked;
      else if (['releaseHour', 'releaseMinute', 'warmupSeconds', 'pollIntervalMs', 'reloadIntervalMs'].includes(key)) config[key] = Number(target.value || DEFAULT_CONFIG[key]);
      else config[key] = String(target.value || '').split(',').map((item) => item.trim()).filter(Boolean);
  ...
  function ensureStyle() {
    if (document.getElementById('glm-coding-guard-style')) return;
    const style = document.createElement('style');
    style.id = 'glm-coding-guard-style';
  ...
  function createPanel() {
    ...
    panel.innerHTML = `
      ...
      <div class="guard-body">
        ...
        <div class="guard-row"><label>开售小时</label><input data-key="releaseHour" type="number" min="0" max="23"></div>
        <div class="guard-row"><label>开售分钟</label><input data-key="releaseMinute" type="number" min="0" max="59"></div>
        <div class="guard-row"><label>提前布防秒数</label><input data-key="warmupSeconds" type="number" min="0" max="300"></div>
        <div class="guard-row"><label>轮询间隔(ms)</label><input data-key="pollIntervalMs" type="number" min="100" max="5000"></div>
        <div class="guard-row"><label>刷新间隔(ms)</label><input data-key="reloadIntervalMs" type="number" min="500" max="10000"></div>
        <div class="guard-switches">
          <div class="guard-switch"><label>启用脚本</label><input data-key="enabled" type="checkbox"></div>
          <div class="guard-switch"><label>窗口内自动刷新</label><input data-key="autoReloadInWindow" type="checkbox"></div>
          <div class="guard-switch"><label>自动点击首个订阅按钮</label><input data-key="autoClickPrimaryButton" type="checkbox"></div>
        </div>
        <div class="guard-row"><label>目标按钮关键词</label><input data-key="clickKeywords" type="text"></div>
        <div class="guard-row"><label>售罄文案关键词</label><input data-key="soldOutKeywords" type="text"></div>
        <div class="guard-row"><label>停止点击关键词</label><input data-key="stopKeywords" type="text"></div>
        <div class="guard-hint">建议只开启到进入订单确认前的自动化，最终支付仍由你手动完成。</div>
    ...
    panel.querySelector('#glm-guard-arm').addEventListener('click', () => {
      ...
      if (config.armed) startLoop();
      else stopLoop();
    });
    panel.querySelector('#glm-guard-check').addEventListener('click', tryAction);
  ...
  function boot() {
    ...
    setInterval(() => {
      createPanel();
      if (config.armed) renderStatus(withinPurchaseWindow() ? '正在抢购窗口内监控。' : '等待下次开售窗口。');
      else renderStatus('扩展已加载，等待布防。');
    }, 1000);
    log('扩展启动完成。');
  }
